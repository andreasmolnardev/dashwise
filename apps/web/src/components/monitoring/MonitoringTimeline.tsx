...
export default function MonitoringTimeline({ status, dateChanged, durationChanged }: MonitoringTimelineProps) {
      ...
      <div className="flex items-stretch gap-1 h-8">
        {segments.map((segment, i) => {
          if (segment.type === "state") {
            const bgColor = segment.status === "up" ? "bg-green-500/40" : "bg-red-500/40";

            return (
              <div
                key={i}
                className={`rounded border border-white/20 ${bgColor} flex items-center justify-center text-sm font-medium`}
          ...
          if (segment.type === "time" && segment.date) {
            return (
              <div
                key={i}
                className="rounded bg-white/10 border border-white/30 flex items-center justify-center px-1 text-sm font-medium whitespace-nowrap overflow-visible"
          ...
          if (segment.type === "now") {
            return (
              <div
                key={i}
                className="rounded bg-blue-500/30 border border-blue-400/50 flex items-center justify-end px-0.5 text-sm font-semibold"
                style={{ minWidth: "14px", maxWidth: "14px" }}
              >
                Now
              </div>
