...
import CreateLinksCollectionDialog from "./CreateLinksCollectionDialog";
import CreateLinksTagDialog from "./CreateLinksTagDialog";
...
export default function LinksLayout({ children }: { children: ReactNode }) {
    ...
    return (
        <ModuleNavigation title="Links" contributions={[
            { kind: "item", id: "home", path: "/links/home", icon: "fa6-solid:house", label: "Home" },
            { kind: "group", id: "lists", group: "Lists", actions: [{ icon: "fa6-solid:plus", title: "Create new list", action: () => { setEditingCollection(null); setCreateListOpen(true); } }] },
            ...userCollections.map((collection): ModuleNavigationContribution => ({ kind: "item", id: `list-${collection.id}`, path: `/links/lists/${collection.id}`, icon: collection.icon || "fa6-solid:folder-open", label: collection.name, group: "Lists", dropdownActions: [{ label: "Edit", icon: "fa6-solid:pen-to-square", action: () => { setEditingCollection(collection); setCreateListOpen(true); } }, { label: "Rename", icon: "fa6-solid:font", action: () => { setRenamingCollection(collection); setRenameListOpen(true); } }] })),
            { kind: "group", id: "tags", group: "Tags", actions: [{ icon: "fa6-solid:plus", title: "Create new tag", action: () => { setEditingTag(null); setCreateTagOpen(true); } }] },
            ...tags.map((tag): ModuleNavigationContribution => ({ kind: "item", id: `tag-${tag.id}`, path: `/links/tags/${tag.id}`, icon: "fa6-solid:hashtag", label: tag.name, group: "Tags", dropdownActions: [{ label: "Edit tag", icon: "fa6-solid:pen-to-square", action: () => { setEditingTag(tag); setCreateTagOpen(true); } }] })),
        ]} overlays={<>
