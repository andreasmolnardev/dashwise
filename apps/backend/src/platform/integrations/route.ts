...
import { IntegrationLifecycleService } from "./lifecycle-service";
...
integrationsRoute
  .get(
    ...
    withJson(async (c) => {
      ...
      return id
        ? lifecycle.get(userId, id, resolveEndpoints)
        : lifecycle.list(userId);
    }),
  )
  .post(
    "/api/v1/integrations",
    withJson(async (c) => {
      ...
      const { userId } = await requireAuth({ token: readAuthToken(c) });
      return lifecycle.create(userId, body ?? {});
    }),
  )
  .put(
    ...
    withJson(async (c) => {
      ...
      const body = await readJsonBody<any>(c);
      const { userId } = await requireAuth({ token: readAuthToken(c) });
      return lifecycle.update(userId, id, body ?? {});
    }),
  )
  .delete(
    "/api/v1/integrations/:id",
    withJson(async (c) => {
      const id = c.req.param("id")!;
      const { userId } = await requireAuth({ token: readAuthToken(c) });
      return lifecycle.remove(userId, id);
    }),
  )
  .post(
    ...
    withJson(async (c) => {
      const body = await readJsonBody<any>(c);
      const { userId } = await requireAuth({ token: readAuthToken(c) });
      return lifecycle.testEndpoint(userId, String(body?.target ?? ""));
    }),
  )
  .post(
  ...
  .get(
    ...
    withJson(async (c) => {
      ...
      if (integrationId) {
        const integration = await lifecycle.get(userId, integrationId);
        if (!integration) {
      ...
      const { integrations } = await lifecycle.list(userId);
