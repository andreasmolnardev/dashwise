...
searchItemsRoute.post("/api/v1/searchItems/usageStats", withJson(async (c) => {
  const body = await readJsonBody<{ id: string; timestamp: string; auth?: any }>(c);
  const { userId } = await requireAuth(body?.auth ?? { token: readAuthToken(c) });
...
searchItemsRoute.get("/api/v1/searchItems/frequentlyUsed", withJson(async (c) => {
  const { userId } = await requireAuth({ token: readAuthToken(c) });
