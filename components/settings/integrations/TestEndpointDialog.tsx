...
export function TestEndpointDialog({
	...
	const curlCommand = useMemo(() => (testResult ? buildCurlCommand(testResult.request) : null), [testResult]);
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-w-3xl frosted text-foreground h-[80vh] overflow-scroll">
				<DialogHeader>
				...
				<div className="space-y-4">
					...
					{testResult ? (
						<Tabs defaultValue="request" className="space-y-4">
							...
							<TabsContent value="request" className="space-y-4 rounded-2xl border border-border/70 bg-background/60 p-4 text-[0.75rem]">
							...
							<TabsContent value="response" className="space-y-4 rounded-2xl border border-border/70 bg-background/60 p-4 text-[0.75rem]">
								<div className="space-y-2">
									<p className="font-semibold">Headers</p>
									<div className="grid gap-1 text-muted-foreground">
								...
								<div className="space-y-1 text-[0.7rem] text-muted-foreground">
									<span>Status</span>
									<span className="font-mono">{testResult.response.status} {testResult.response.statusText}</span>
								</div>
							</TabsContent>
							<TabsContent value="manual" className="space-y-3 rounded-2xl border border-border/70 bg-background/60 p-3 text-[0.75rem]">
								<p className="text-[0.65rem] font-semibold uppercase tracking-wide text-muted-foreground">cURL command</p>
...
function buildCurlCommand(request: EndpointTestResult["request"]) {
	...
	for (const [key, value] of Object.entries(request.headers ?? {})) {
		...
		const formattedValue = redactBearerHeader(key, value);
		tokens.push({ text: "-H", quoted: false });
		tokens.push({ text: `${key}: ${formattedValue}`, quoted: true });
	}
...
function redactBearerHeader(key: string, value: string) {
