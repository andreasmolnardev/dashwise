...
export function AddIntegrationConfigDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl frosted text-(--text-primary)">
        <DialogHeader>
                ...
                {visibleEnvFields.map((field) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs">
                      <span>{field.key}</span>
                      {field.required ? (
                        <span className="text-destructive-foreground">required</span>
                      ) : null}
                    </div>
                    <Input
                      ...
                      onChange={(event) =>
                        onEnvironmentOverrideChange(field.key, event.target.value)
                      }
                    />
                    {field.description ? <p className="text-[0.75rem]">{field.description}</p> : null}
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm">
                Paste a config JSON or YAML that declares environment_variables to show override
                inputs here.
              </p>
            )}
          </div>
