...
export default function ItemDetailsCard({ resolved, className, formatters }: ItemDetailsCardProps) {
  ...
  return (
    <div className={`frosted rounded-lg p-3 ${className ?? ""}`}>
      {header?.show !== false && header?.title && (
        ...
        >
          {header.icon ? <AppIcon source={header.icon} className="h-4 mx-0.5" alt="" size={16} /> : null}
      ...
      <div className="flex items-center gap-x-3 gap-y-1">
        {iconSrc ? (
          <div className="row-span-2 flex h-12 w-12 items-center justify-center rounded-lg shrink-0">
            <AppIcon source={iconSrc} className="h-9 w-9 object-contain" alt="" size={36} />
          </div>
        ) : null}
